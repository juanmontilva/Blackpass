...
$totalv = 0;
				...
				$pie_sql= "SELECT SUM(p.monto) as monto , x.pais, l.localidad 
							FROM apps_pay_o p, apps_localidades l, apps_paises x
							WHERE p.idl = l.id_loc
							and l.id_pais = x.cod
							and p.fecha BETWEEN '".$fechas[0]."' and '".$fechas[1]."'
							GROUP BY l.localidad, x.pais";
			//echo $pie_sql;
			 ...
			 while($resul =  mysqli_fetch_assoc($query_u2)){
						...
						$datos_pie .= "{name:'".$resul['localidad']."',y:".$monto."},";
			}
...
<div class="row">
	<div class="col-xs-12">
		<div class="box">
			...
			<tbody>
				<?php
				  $sql = mysqli_query($con,"SELECT SUM(p.monto) as monto , x.pais, l.localidad 
							FROM apps_pay_o p, apps_localidades l, apps_paises x
							WHERE p.idl = l.id_loc
							and l.id_pais = x.cod
							and p.fecha BETWEEN '".$fechas[0]."' and '".$fechas[1]."'
		...
		<div class="box">
			<div class="box-header">
				<div class="box-name">
					<i class="fa fa-shopping-cart"></i>
					<span>Distribución por Localidades <strong><?php $year." - ".$mes?>  </strong> </span>
				</div>
				<div class="box-icons">
					<a class="collapse-link">
						<i class="fa fa-chevron-up"></i>
...
function consulta_f(){
  ...
  //alert (dataString);return false;
		  $.ajax({
			type: "GET",
			url: "home.php#ajax/mod_bi/list_loc.php",
			data: dataString,
			success: function() {
			  $("#ajax-content").css({
                "opacity": 0.4
				});
			$("#ajax-content").load("ajax/mod_bi/list_loc.php?ini="+ini+"&fin="+fin+"&mes_b="+mes, function(e){
						 $("#ajax-content").css({
							"opacity": 1
						});
					});		  
