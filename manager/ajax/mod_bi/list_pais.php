...
<div class="row">
	<div id="breadcrumb" class="col-md-12">
		<ol class="breadcrumb">
			...
			<li><a href="home.php#ajax/mod_bi/list_pais.php">Ingresos por Productos</a></li>
		</ol>
	</div>
</div>
<div class="row">
	...
	<div class="col-xs-12">
		<div class="box">
			<div class="box-header">
				<div class="box-name">
					<i class="fa fa-shopping-cart"></i>
					<span>Ingresos por Productos <strong><?php echo $year." - ".$mes?>  </strong></span>
				</div>
				<div class="box-icons">
					<a class="collapse-link">
						<i class="fa fa-chevron-up"></i>
			...
			<tbody>
				<?php
				  $sql = mysqli_query($con,"SELECT SUM(p.monto) as venta,  m.marca
							FROM  apps_pagos p, 
							apps_clientes c, apps_marcas m
							WHERE 
							p.idc = c.id
			...
			<br>
			<table class="table table-bordered table-striped table-hover table-heading table-datatable" id="datatable-6">
			<thead>
				<tr>
					<th>Producto</th>
					<th>Sin Validar</th>
			...
			<tbody>
				<?php
				  $sql2 = mysqli_query($con,"SELECT SUM(p.monto) as venta,  m.marca
							FROM  apps_pagos p, 
							apps_clientes c, apps_marcas m
							WHERE 
							p.idc = c.id
	...
	<div class="col-xs-6">
		<div class="box">
			<div class="box-header">
				<div class="box-name">
					<i class="fa fa-shopping-cart"></i>
					<span>Distribución de Ingresos <strong><?php $year." - ".$mes?>  </strong> </span>
				</div>
				<div class="box-icons">
					<a class="collapse-link">
						<i class="fa fa-chevron-up"></i>
		...
		<div class="box">
			<div class="box-header">
				<div class="box-name">
					<i class="fa fa-shopping-cart"></i>
					<span>Ingresos <strong><?php $year." - ".$mes?>  </strong> </span>
				</div>
				<div class="box-icons">
					<a class="collapse-link">
						<i class="fa fa-chevron-up"></i>
...
function consulta_f(){
  ...
  //alert (dataString);return false;
		  $.ajax({
			type: "GET",
			url: "home.php#ajax/mod_bi/list_pais.php",
			data: dataString,
			success: function() {
			  $("#ajax-content").css({
                "opacity": 0.4
				});
			$("#ajax-content").load("ajax/mod_bi/list_pais.php?ini="+ini+"&fin="+fin+"&mes_b="+mes, function(e){
						 $("#ajax-content").css({
							"opacity": 1
						});
					});		  
...
$(document).ready(function() {
    ...
    yAxis: {
        title: {
            text: 'Total Ingresos Registrados'
        }
    ...
    series: [
        {
            ...
            colorByPoint: true,
            data: [
			<?php while($rows = mysqli_fetch_assoc($sqls)){
				$sqlv = mysqli_query($con,"select  SUM(p.monto) as totalv
										from apps_pagos p, apps_marcas m, apps_clientes c
										where c.id = p.idc
					...
					if(mysqli_num_rows($sqlv)!=0){
						$totales = intval($rowv['totalv']);
					}
					//echo "total = ".$totales." - ".$totalv;
					$porc =  ($rows['venta']*100)/$totales;
					//echo $porc;
				?>
                {
                    name: "<?php echo $rows['marca'];?>",
                    y: <?php echo $porc;?>,
                    drilldown: "<?php echo $rows['marca'];?>"
                },
			<?php }?>
...
Highcharts.chart('label_sales', {
    ...
    series: [
	<?php 
		$sql4 = mysqli_query($con,"select * from  apps_marcas");
		$l=0;
		while($row4 = mysqli_fetch_assoc($sql4)){
			$l++;
			$sql5 = mysqli_query($con,"select  SUM(p.monto) as factu, m.marca,p.fecha
							from apps_pagos p, apps_marcas m, apps_clientes c
							where c.id = p.idc
							and c.plan = m.id_marca
							and p.fecha  BETWEEN '".$fechas[0]."' and '".$fechas[1]."'
							and m.id_marca = '".$row4['id_marca']."'  
							and p.status = 2
							group by m.marca,p.fecha
