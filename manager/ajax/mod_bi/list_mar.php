	...
	<div class="col-xs-12">
		<div class="box">
			<div class="box-header">
				<div class="box-name">
					<i class="fa fa-shopping-cart"></i>
					<span>Reporte Cobros por Empleados <?php echo $fechas[0]." al ".$fechas[1];?></span>
				</div>
				<div class="box-icons">
			...
			<div style="margin-left:87%">
			 		...
			 		<?php 
						...
						while($resultados = mysqli_fetch_assoc($query_u)){
							...
							$localid = mysqli_query($con,"select p.*, l.localidad from
				...
				<table class="table table-bordered table-striped table-hover table-heading table-datatable" id="datatable-<?php echo $b;?>">
					<thead>
						<tr>
							...
							<th>Ubicación<i class="fa fa-bookmark"></i></th>
					...
					<?php 
						...
						<tr>
							...
							<td><?php echo strtoupper($row2['localidad']);?></td>
							<td><?php echo number_format($row2['monto'],2 ,",", ".");?> USD </td>
							<td><button type="button" class="btn btn-default btn-primary" onclick="ver_grafica('<?php echo ($resultados['id_user']);?>','<?php echo utf8_encode($operador);?>','<?php echo $fechas[0];?>','<?php echo $fechas[1];?>');">Graficar <i class="fa fa-chart"></i></span></button></td>
						</tr>
						<?php 
						}
						?>	
	...
	<div class="col-xs-12">
		<div class="box">
			<div class="box-header">
				<div class="box-name">
					<i class="fa fa-shopping-cart"></i>
					<span>Cobros por Empleados  <strong><?php $year." - ".$mes?>  </strong> </span>
				</div>
				<div class="box-icons">
					<a class="collapse-link">
						<i class="fa fa-chevron-up"></i>
...
function consulta_f(){
  ...
  //alert (dataString);return false;
		  $.ajax({
			type: "GET",
			url: "home.php#ajax/mod_bi/list_mar.php",
			data: dataString,
			success: function() {
			  $("#ajax-content").css({
                "opacity": 0.4
				});
			$("#ajax-content").load("ajax/mod_bi/list_mar.php?ini="+ini+"&fin="+fin+"&mes_b="+mes, function(e){
						 $("#ajax-content").css({
							"opacity": 1
						});
					});		  
...
Highcharts.chart('label_sales', {
	...
	<?php 
		...
		while($row4 = mysqli_fetch_assoc($sql4)){
			...
			$sql5 = mysqli_query($con,"select count(p.id) as cantidad, sum(p.monto) as total
							from
							apps_pay_o p, apps_localidades l 
							where p.operador = '".$operador."'
							and p.fecha BETWEEN  '".$fechas[0]."' and '".$fechas[1]."'
...
$('#loading_home8').hide(5000);
	function graf_employer(x,y,z,e){
		var datos = "";	
		$.ajax({
			url:'ajax/mod_bi/ventas_x_emplyer.php',
			data:{
				"identificador":x,
				'start':y,
				'end':z,
