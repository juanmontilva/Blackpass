...
//echo "Conexión exitosa!";
		...
		if($accionT=="group_data"){	
			...
			$sql = "SELECT e.*, s.precio FROM  events e, apps_servicios_d s
							WHERE 
						    start BETWEEN  '".$s[0]."' and '".$s[1]."'
							and e.idh = s.idh ".$where." " ;
			$result = mysqli_query($mysqli, $sql);
			//echo $sql;
			$paga = $prog = $canc = $conf = $i = $dia = 0;
			$totalm = $proyec = $perdi = $confir = 0;
			while($row2 = mysqli_fetch_assoc($result)) {
				if($row2['status']==0){
					$prog = $prog+1; 
					$proyec = $proyec + $row2['precio'];
				}else if($row2['status']==4){
					$canc = $canc+1; 
					$perdi = $perdi + $row2['precio'];
				}else if($row2['status']==1){
					$conf = $conf+1; 
					$confir = $confir + $row2['precio'];
				}else if($row2['status']==2){
					$paga = $paga+1; 
					$confir = $confir + $row2['precio'];
				}
				$h = explode(" ",$row2['start']);
				//echo $h[0]."<br>";
				//echo "hoy es: ".$dias."<br>";
		...
		if($accionT=="update_c"){
			$cl = mysqli_real_escape_string($mysqli,(strip_tags($_GET['cli'],ENT_QUOTES)));
			$sql = "SELECT e.* FROM  events e WHERE localizador = '".$cl."'" ;
			$result = mysqli_query($mysqli, $sql);
			if(mysqli_num_rows($result)!=0){
				$update = mysqli_query($mysqli,"UPDATE events set status = 2 where localizador = '".$cl."'");
				if($update){
					header('Content-type: application/json; charset=utf-8');
			//echo json_encode($menu);
