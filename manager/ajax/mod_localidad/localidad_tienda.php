...
$html = "";
	$local= mysqli_query($con,"select * from apps_localidades where id_provincia = $id_category order BY localidad ASC");

    while($resultados = mysqli_fetch_assoc($local)){               
        $html .= '<option value="'.$resultados['id_loc'].'">'.utf8_encode($resultados['localidad']).'</option>';
    }
