...
include("24hopen.php");
$APIurl = 'https://api.chat-api.com/instance215059/';
 ...
 if($row_b['tipo']==0){ // proceso cancelar cita
	...
	if($row_b['status']==0){
		...
		sendMessage($chatId,$mensaje);
					...
					$mensaje = "📋 Disculpa no laboramos los dias *Domingo* \n\n".
					"_Intenta nuevamente_\n".
					"_Ejemplo: 🗓️ *".$hoy."*_ \n\n".
					"*AYUDA ó HELP*  👉🏻 Para más comandos";
					$respuesta = sendMessage($chatId,$mensaje);
...
if(strtoupper($cuerpo) =='LG'){
	$respuesta = "*Envia / Send*:\n\n".
	             "*ESP*  👉🏻 Español\n".
				 "*ING*  👉🏻 English";
		sendMessage($chatId,$respuesta);
}else if(strtoupper($cuerpo) =='ING' || strtoupper($cuerpo) =='INGLISH' || strtoupper($cuerpo) =='ENGLISH' || strtoupper($cuerpo) =='INGLES'){
	$respuesta = cambiar_idioma($chatId,2,$numero);
	if($respuesta!=""){
		$respuesta = "🇺🇸 *OK!* _We have successfully changed your language_";
		sendMessage($chatId,$respuesta);
	}
}else if(strtoupper($cuerpo) =='ESP' || strtoupper($cuerpo) =='ESPAÑOL' || strtoupper($cuerpo) =='ESPANOL' || strtoupper($cuerpo) =='SPANISH'){
	...
	sendFile($chatId,$banner[1],$banner[0]);
		...
		$mensaje =  "🛍️ *".utf8_encode($row['titulo'])."* \n\n".
								...
								"*PC*      👉🏻  _Pagar Cita_ 💲\n".
								"*ADS*   👉🏻    _Ofertas_ 🏷️ \n".
								"*ATC*   👉🏻    _Ser atendido por un asesor_ 🏷️ \n\n".
								"Recuerda que el formato de Fechas es:\n".
								"Ejemplo: *M/d/Y ".date("M/d/Y")."*\n".
								"Enero      👉🏻   Jan \n".
								"Febrero   👉🏻   Feb \n".
								"Marzo     👉🏻   Mar \n".
								"Abril        👉🏻    Apr \n".
	...
	}else if($message['body']!='' && $message['body']!='S' && $message['body']!='L' && strtoupper($message['body'])!='MF' && strtoupper($message['body'])!='ADS' && strtoupper($message['body'])!='NOW'){
...
if($consultaw[1]==3 && $consultaw[0]!=""){
	$respuesta = sendMessage($chatId,$consultaw[0]);
	$obj = json_decode($respuesta);
	if($obj->{'sent'}==1){
		$insert_aux = mysqli_query($con,"INSERT INTO aux_clientes (chatid ,nombre,cumple,gender,paso)
								VALUES ('$chatId','0','01/01/1970','1','1') ");
	...
	$respuesta = sendMessage($chatId,$consultaw[0]);	
		...
		if(mysqli_num_rows($sql_1)!=0){
			...
			$insert = mysqli_query($con,"INSERT INTO apps_clientes (codigo,nombres,fecha_nacimiento,sexo,telefono,estado)
						VALUES ('".$phone[0]."','".$row_i['nombre']."','".$row_i['cumple']."',
						'".$row_i['gender']."','".$phone[0]."','1') ");
			if($insert){
				$respuesta = sendMessage($chatId,$consultaw[0]);
				$obj = json_decode($respuesta);
				if($obj->{'sent'}==1){
