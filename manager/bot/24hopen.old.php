					...
					if(mysqli_num_rows($sql_c) >= 1){
						while($row_c=mysqli_fetch_assoc($sql_c)){
							...
							$sql_l = mysqli_query($con,"SELECT * FROM apps_localidades 
													where id_provincia = '".$row_c['codprovincia']."'");
...
$sql_x = mysqli_query($con,"select w.id,a.localidad from  aux_local a, apps_welcome w
						where a.idw = w.id
						and w.chat_id = '$y' ");	
...
function encuesta_anul($y,$x){
	...
	$orden  = "";
	if($x==1 || $x==2 || $x==3 || $x==4){
		$sqlx = mysqli_query($con,"select * from apps_reservas_anu where chatid = '".$y."' and tipo = 0");
...
function cita_modf($y,$x,$numero){
	...
	if($validar == 1){
		...
		if($dias_t == 0){
			...
			$sql_ = mysqli_query($con,"select * from apps_reservas_anu where chatid = '$y' and tipo = 1 ");
			$g = "select * from apps_reservas_anu where chatid = '$y' and tipo = 1 ";
