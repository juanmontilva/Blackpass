...
if(mysqli_num_rows($sqld)==1){
	$rowt = mysqli_fetch_assoc($sqld);
	$tar = $rowt['descripcion'];
}else{
	$tar = 0;
}
$semana = semana_i();

$sqlc = mysqli_query($con,"SELECT p.*
						FROM apps_pagos p
...
<head>
  ...
  <link href="../assets/css/nucleo-svg.css" rel="stylesheet" />
   ...
   <link href="../assets/css/sweetalert/dist/sweetalert.css" rel="stylesheet" type="text/css" defer >
  <!-- Font Awesome Icons -->
...
<br>
    ...
    <div class="container-fluid py-4">
      ...
      <div class="row">
        ...
        <div class="container">
			...
			<div id="load_plan" style="display:none;">
				<img src="../assets/img/loading.gif">
			  </div>
		
            <div style="display:block;" id="list_invoice" class="row justify-content-center">
                <div class="col-lg-12">
          <div class="card h-100">
            <div class="card-header pb-0 p-3">
              <div class="row">
                <div class="col-6 d-flex align-items-center">
                  <h6 class="mb-0">Lista de Recargas</h6>
                </div>
                <div class="col-6 text-end">
                  <button onclick="start_r(<?php echo $idu;?>,<?php echo $token;?>)" class="btn bg-gradient-dark btn-sm mb-0">Nueva Recarga</button>
                </div>
              </div>
            </div>
				  ...
				  ?>
                    <tr>
                      <td>
                        <div class="d-flex px-2 py-1">
                          ...
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm"><?php echo $r2['orden'];?></h6>
                          </div>
                        </div>
                      </td>
                      <td>
                      ...
                      <td class="align-middle text-center text-sm">
                        <span class="badge badge-sm bg-gradient-info"><?php echo $r2['monto'];?></span>
                      </td>
                      <td class="align-middle text-center">
					  <?php if($r2['status']==2){?>
                        <span class="badge badge-sm bg-gradient-success"><?php echo $status;?></span>
					  <?php }else if($r2['status']==1){?>
						<span class="badge badge-sm bg-gradient-dark"><?php echo $status;?></span>
					  <?php }else if($r2['status']==0){?>	
						<span class="badge badge-sm bg-gradient-danger"><?php echo $status;?></span>
                      <?php }?>
					  </td>
                    </tr>
					<?php
  ...
  <div class="modal-dialog modal-xs">
    <div class="modal-content">
      <div class="modal-header">
	     <button type="button" class="close" onclick="cerrar_tax2();" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="form_cambio_modal"><i class="fa fa-globe"></i> Agregar Recarga</h4>
      </div>
      <div class="modal-body">
		<div class="box-content no-padding">
		<div id="load_r" style="display:none;">
				<img src="../assets/img/loading.gif">
	   </div>
				...
				<form id="form_upd_cambio" name ="form_upd_cambio" method="post"  class="form-horizontal">
					<fieldset>
						...
						<div class="form-group" id="titular">
							...
							<div class="col-sm-5">
								<input type="text" readonly value = "XXXX-XXXX-XXXX-<?php echo $tar;?>" class="form-control" name="tarjeta" id="tarjeta" data-toggle="tooltip" data-placement="bottom" title="Tarjeta"> 
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								<select class="form-control" id="tpc" name ="tpc" onchange="tipo_pago();" >
								<option value="">----</option>
								<option value="1">Zelle</option>
								<option value="2">Efectivo</option>
								<option value="3">TDC/TDD</option>
								<option value="6">Transferencia</option>
								<option value="7">Pagomovil</option>
								</select>
							</div>
						</div>
						...
						<div class="form-group" id="nombre_t" style="display:none;">
						...
						<div class="form-group" id="lmonto">
							<label class="col-sm-3 control-label">Monto</label>
							<div class="col-sm-5">
								<input type="number"  class="form-control" name="montos" id="montos" data-toggle="tooltip" data-placement="bottom" title="Monto"> 
							</div>
						</div>
						<div id = "cash" style="display:none;" class="col-sm-5">
								<select id="envio" name ="envio" class="form-control" onchange="validar_forma();" >
								<option value="1">Enviar Efectivo</option>
								<option value="2">Mandar a Buscar</option>
								</select>
						</div>
					
						<div style="display:none" id="senvio">
						<div class="form-group">
							<label class="col-sm-3 control-label">Dirección</label>
							<div class="col-sm-5">
								<input type="text"  class="form-control" name="dir_rec" id="dir_rec" data-toggle="tooltip" data-placement="bottom" title="Dirección"> 
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label">Contacto</label>
							<div class="col-sm-5">
								<input type="text"  class="form-control" name="contacto" id="contacto" data-toggle="tooltip" data-placement="bottom" title="Contacto"> 
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label">Telefono</label>
							<div class="col-sm-5">
								<input type="number"  class="form-control" name="phone" id="phone" data-toggle="tooltip" data-placement="bottom" title="Contacto"> 
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label">hora</label>
							<div class="col-sm-5">
								<input type="time"  class="form-control" name="hora" id="hora" data-toggle="tooltip" data-placement="bottom" title="08:00"> 
							</div>
						</div>
						</div>
						<br>
					...
					<div class="form-group">
						...
						<input type="hidden" id="client" name="client" value ="<?php echo cadena();?>">
						<input type="hidden" id="codigo" name="codigo" value ="">
						<input type="hidden" id="ncliente" name="ncliente" value ="">
						<button type="button" id="btn_save" class="btn btn-lg btn-dark btn-label-right" onclick="save_pago_res();">Guardar Recarga</button>
						</div>
					</div>
				</form>
				
					<div style="display:none" id="p2p">
						<form method='POST' target="_blank" action='https://www13.bfc.com.ve:1544/'>
							
							   <div class="form-group">
									<label class="col-sm-3 control-label">Número de orden</label>
									<div class="col-sm-5">
										<input type="text" value = "0123456" readonly  class="form-control" name="NROORDEN" id="NROORDEN" data-toggle="tooltip" data-placement="bottom" title="ORDEN"> 
									</div>
								</div>
							  <div class="form-group">
									<label class="col-sm-3 control-label">Monto</label>
									<div class="col-sm-5">
										<input type="number" step=".01"  class="form-control" name="MONTOBOTON" id="MONTOBOTON" data-toggle="tooltip" data-placement="bottom" title="Monto"> 
									</div>
								</div>
						
						<!-- URL REDIRECT ES LA URL DENTRO DE SU SITIO WEB A DONDE SE ENVIARÁN LOS DATOS DE PAGO EXITOSO O FALLIDO -->
						<!-- si su url es tusitio.com/pagoexitoso.php el value de urlRedirect debe ser pagoexitoso.php -->
						<!-- si su url es tusitio.com/subcarpeta/pagoexitoso.php el value de urlRedirect debe ser subcarpeta/pagoexitoso.php -->
							  <input type='hidden' name='urlRedirec' value = "http://app.24hopen.com/ve/blackpass/script/pagoexitoso.php?xyz=p2p&idc=<?php echo $encriptar($_SESSION['xy'])?>" required >

						<!-- PARAMETROS DEL SERVICIO (NO MODIFICIAR) -->
							  <input type='hidden' value='SiwzMTM1MzgyODAsNDI0MjMxNDA4NCxJTlZFUlNJT05FU19BLkouTS5fMTk2OSxKMzEzNTM4MjgwLnBuZw==' name='KEYID'>
							  <input type='hidden' value='MjRob3Blbi5jb20sMjRob3Blbi5jb20sSiwzMTM1MzgyODAsNDI0MjMxNDA4NCwyMDIzMTAxMyw4MDMxOQ==' name='PRIVATEKEY'>
							  <input type='hidden' value='XzdgN39/QikQw6B7Ojpvnf6O.L1ATv8pA4JsXxfEgXLDev+qdMKcJyrxoeWqv8a3Zc6ltsmTMv+5Q/KjAp0RLpF1rlq7w.mpbRGnr1levVxvIgWLPXvuCd' name='JWT'>
							  <input type='hidden' value='B' name='MODOPAGO'>
						
							  <button type="submit" id="btn_save" class="btn btn-lg btn-dark btn-label-right" onclick="save_pago_res();">Pagar</button>
						</form>
						</div>
			</div>
		</div>
 ...
 // view_plan(sessionStorage.uid);
  ...
  <script type="text/javascript" language="javascript" defer src="../assets/css/sweetalert/dist/sweetalert.min.js"></script>
</body>
