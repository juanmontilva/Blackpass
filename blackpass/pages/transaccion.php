...
$sqlc = mysqli_query($con,"SELECT p.*
						FROM apps_yyy p
						WHERE p.send = '".$_SESSION['xy']."' or receive = '".$_SESSION['xy']."' 
						ORDER BY fecha DESC");
...
<br>
            ...
            <div class="card-header pb-0 p-3">
              <div class="row">
                <div class="col-6 d-flex align-items-center">
                  <h6 class="mb-0">Mis Movimientos</h6>
                </div>
                <div class="col-6 text-end">
                  <button onclick="start_t(<?php echo $idu;?>,<?php echo $token;?>)" class="btn bg-gradient-dark btn-sm mb-0">Transferir Saldo</button>
                </div>
              </div>
            </div>
                ...
                <table class="table align-items-center mb-0">
                  <thead>
                    <tr>
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Transacción</th>
					  ...
					  <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Destinatario</th>
				  ...
				  <?php
						if(mysqli_num_rows($sqlc)!=0){
							...
							while($r2 = mysqli_fetch_assoc($sqlc) ){
								...
								if($r2['send']==$_SESSION['xy']){
									$tipo = "Enviada";
									$tp = "1";
									$sqlt = mysqli_query($con,"select nombres from apps_clientes
									where id = '".$r2['receive']."'");
									if(mysqli_num_rows($sqlt)!=0){
										$r3 = mysqli_fetch_assoc($sqlt);
										$destino = $r3['nombres'];
									}else{
										$destino = "NO DISPONIBLE";
									}
									
								}else if($r2['receive']==$_SESSION['xy']){
									$tipo = "Recibida";
									$tp = "2";
									$sqlt = mysqli_query($con,"select nombres from apps_clientes
									where id = '".$r2['send']."'");
				  ...
				  ?>
                    <tr>
					  ...
					  <td class="align-middle text-center text-sm">
                        <?php if($tp==1){?>
                        <span class="badge badge-sm bg-gradient-secondary"><?php echo $tipo;?></span>
					  <?php }else if($tp==2){?>
						<span class="badge badge-sm bg-gradient-primary"><?php echo $tipo;?></span>
					    <?php }?>
                      </td>
  ...
  <div class="modal" id="form_tf_modal_add" tabindex="-1" role="dialog" aria-labelledby="form_cambio_modal_add" aria-hidden="true">
  <div class="modal-dialog modal-xs">
    <div class="modal-content">
      <div class="modal-header">
	     <button type="button" class="close" onclick="cerrar_tax3();" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="form_tf_modal"><i class="fa fa-globe"></i> Transferir Saldo</h4>
      </div>
      <div class="modal-body">
		...
		<div id="load_r" style="display:none;">
				...
				<form id="form_tf_cambio" name ="form_tf_cambio" method="post"  class="form-horizontal">
					<fieldset>
						<legend>Transferir Saldo</legend>
						<div class="form-group" id="titular">
							<label class="col-sm-3 control-label">Numero Tarjeta</label>
							<div class="col-sm-5">
								<input type="text" maxlength="16" placeholder = "XXXXXXXXXXXX-<?php echo $tar;?>" class="form-control" name="tarjeta" id="tarjeta" data-toggle="tooltip" data-placement="bottom" title="Tarjeta"> 
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								...
								<option value="">----</option>
								<option value="4">Mi Saldo</option>
					...
					<div class="form-group">
						...
						<button type="button" id="btn_save2" class="btn btn-lg btn-dark btn-label-right" onclick="save_tf_res();">Enviar Dinero</button>
						</div>
					</div>
				</form>
			</div>
