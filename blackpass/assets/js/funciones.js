...
function edit_p(){
		$("#form_perf_modal_add").show();
		$("#form_perf_modal_add").modal({
		show: true,
		backdrop: 'static',
		keyboard: false,
		history: false,
...
function cerrar_tax7(){
	$("#form_perf_modal_add").hide();
	$('#form_de_cambio')[0].reset();
	$("#form_perf_modal_add").modal('hide');//ocultamos el modal
	$('body').removeClass('modal-open');//eliminamos la clase del body para poder hacer scroll
	$('.modal-backdrop').remove();//eliminamos el backdrop del modal
}
		...
		function upd_cliente(e){
			...
			if(t_mail==""){
			 ...
			 return;	
			    ...
			    beforeSend: function(){
							$("#load_r3").show();
							$("#btnFetch3").prop("disabled", true);
							},
			    success : function(json) {
					$("#load_r3").hide();
					$("#btnFetch3").prop("disabled", false);					
					if(json.response=='ok'){
						$("#div_msg3").hide();
						$('#form_de_cambio')[0].reset();
						$("#form_perf_modal_add").hide();
						...
						refresh_list_tf3();
							...
							$("#div_msg3").show();
							$("#div_msg3").html("<div class='form-group'><div class='col-sm-12'><div class='alert alert-danger'>ERROR al realizar la suscripción</div></div></div>");
					}else if(json.response=="ex"){

							$("#div_msg3").show();
							$("#div_msg3").html("<div class='form-group'><div class='col-sm-12'><div class='alert alert-danger'>El Cliente ya existe</div></div></div>");
					}else {
...
function refresh_list_tf3(){
	window.location.href = "profile.php?"+rand_code(caracteres,longitud);
}
